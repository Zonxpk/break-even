-- Trusted public-release flows. Sensitive state changes happen behind RPCs.
create table public.progress_events (
  id         bigint generated by default as identity primary key,
  user_id    uuid not null references auth.users (id) on delete cascade,
  event      text not null check (event in ('swipe_rejected', 'story_beat')),
  created_at timestamptz not null default now()
);

alter table public.progress_events enable row level security;
-- No table grants: clients reach this table only through award_progress().
create index progress_events_user_day_idx
  on public.progress_events (user_id, event, created_at desc);

create or replace function public.award_progress(p_event text)
...
begin
  ...
  case p_event
    when 'swipe_rejected' then v_xp := 2;  v_limit := 10;
    when 'story_beat'     then v_xp := 10; v_limit := 20;
    else raise exception 'INVALID_PROGRESS_EVENT';
  end case;

  select count(*) into v_count
    from public.progress_events
   where user_id = v_uid and event = p_event
  ...
  insert into public.progress_events (user_id, event) values (v_uid, p_event);
...
revoke all on function public.award_progress(text) from public, anon;
grant execute on function public.award_progress(text) to authenticated;
