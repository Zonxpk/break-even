---
type: "meta"
title: "Wiki Index"
status: "active"
tags: ["index"]
created: "2026-06-13"
updated: "2026-06-28"
---

# geemong Wiki

Mode B — Repository architecture wiki. Seeded from `/understand` knowledge graph.

## Navigation

- [[overview]]
- [[roadmap]] — เล่นสนุก → ไวรัล → สปอนเซอร์ (HTML: `prototypes/roadmap.html`)
- [[hot]] — recent context (~500 words)
- [[log]] — operation history

## Architecture

### Modules (layers)
- [[modules/presentation-layer|Presentation Layer]]
- [[modules/domain-logic-layer|Domain Logic Layer]]
- [[modules/data-access-layer|Data Access Layer]]
- [[modules/types-layer|Types Layer]]
- [[modules/backend-layer|Backend Layer]]
- [[modules/test-layer|Test Layer]]
- [[modules/documentation-layer|Documentation Layer]]
- [[modules/configuration-layer|Configuration Layer]]
- [[modules/infrastructure-layer|Infrastructure Layer]]

### Components (features)
- [[components/gag-engine|Gag Tracking Engine]]
- [[components/dating-sim|Dating Sim]]
- [[components/dating-ui-polish|Dating UI Polish]]
- [[components/merch-shop|Merch Shop]]
- [[components/mobile-api|Mobile API Layer]]
- [[components/supabase-backend|Supabase Backend]]

### Flows (guided tour)
- [[flows/01-entry-point-boot-the-parody-super-app|1. Boot the app]]
- [[flows/02-the-super-app-home-screen|2. Home screen]]
- [[flows/03-browse-place-a-doomed-order|3. Place order]]
- [[flows/04-order-lifecycle-api|4. Order API]]
- [[flows/05-live-tracking-the-gag-experience|5. Live tracking]]
- [[flows/06-the-gag-tracking-engine|6. Gag engine]]
- [[flows/07-failure-finale-loyalty-rewards|7. Failure & rewards]]
- [[flows/08-dating-sim-daily-swipe-deck|8. Dating swipe]]
- [[flows/09-dating-sim-chat-beats-date-orders|9. Dating chat]]
- [[flows/10-merch-pop-up-shop|10. Merch shop]]
- [[flows/11-brand-booth-partner-tie-ins|11. Brand & partners]]
- [[flows/12-supabase-backend-local-stack-schema|12. Backend]]

## Design

- [[concepts/doodle-design-language|Doodle Design Language (เบรค, อีเวน)]] — **current visual direction**
- [[concepts/dating-experience-variants|Dating Experience Variants]] — 3 Doodle prototypes (`prototypes/dating-sim/`)

## Marketing

- [[marketing/marketing-plan|Marketing Plan — 12 เดือนที่ของไม่เคยถึง]] — in-world AARRR plan (satire)
- [[marketing/landing-page|Break E'Wen Landing Page]] — Doodle parody one-pager (`prototypes/landing/`)

## Reference

- [[dependencies/tech-stack|Tech Stack]]
- [[sources/understand-knowledge-graph|Understand source graph]]
- [[sources/breakeven-doodle-prototype|Doodle prototype (source)]]
