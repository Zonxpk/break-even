...
Matched personas appear in `dating/matches.tsx` (polished `MatchRow` list) and open into `dating/chat/[matchId].tsx` — affection bar, chat bubbles, story beats, and SpotPicker date booking. Chat history persists locally via `chatStorage.ts` and `chatSession.ts`. Persona replies come from optional BYO Anthropic API (`llm.ts`) with `chat.ts` scripted fallback. Story beats unlock by affection threshold; `affectionDrip.ts` caps per-session gains. Users can book a 'date' service order through `bookDate.ts`, which reuses the doomed delivery engine — and failed date orders inject a persona apology back into chat.

## Files in this step

- `mobile/src/app/dating/matches.tsx` — Match list with avatar, affection pill, empty state.
- `mobile/src/app/dating/chat/[matchId].tsx` — Chat UI: `AffectionBar`, `ChatBubble`, beats panel, date CTA, SpotPicker.
- `mobile/src/dating/ui/MatchRow.tsx`, `AffectionBar.tsx`, `ChatBubble.tsx` — shared presentation components.
